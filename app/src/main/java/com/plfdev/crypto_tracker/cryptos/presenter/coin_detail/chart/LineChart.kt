...
fun LineChart(
    ...
    style: ChartStyle,
    /*
        This is a way to show only a range of data points on the screen. Also can include scroll but
        it will show only the range specified
     */
    ...
    /*
        remember (selectedDataPoint) it will call when selectedDataPoint changes.
        And it will be the same for the other values above
     */
    ...
    Canvas(
        ...
        //GET THE MAX HEIGHT AVAILABLE - MARKERS LEFT AND BOTTOM CONSIDERING THE SPACING
        ...
        //2659 - 2583 / labelCountExcludingLastLabel
...
//        )
        ...
        yLabelTextLayoutResults.fastForEachIndexed { index, result ->
            ...
            drawPoints = visibleDataPointsIndices.map {
                val xDataPoint = viewPortLeftX + (
                    it - visibleDataPointsIndices.first
                ) * xLabelWidth + xLabelWidth / 2f

                // [minYValue; maxYvalue] -> [0; 1]
                // [0; 1000]
                val ratio = (dataPoints[it].y - minYValue) / (maxYValue - minYValue)
                val yDataPoint = viewPortBottomY - (ratio * viewPortHeightPx)
            ...
            for (i in 1 until drawPoints.size) {
                ...
                val xCP = (p1.x + p0.x) / 2f
                val y1 = p0.y
                val y2 = p1.y

                connectionPoints1.add(DataPoint(xCP, y1,""))
                connectionPoints2.add(DataPoint(xCP, y2,""))
            }
            ...
            val linePath = Path().apply {
                if(drawPoints.isNotEmpty()) {
                    ...
                    for(z in 1 until drawPoints.size) {
                        cubicTo(
                            x1 = connectionPoints1[z - 1].x,
                            y1 = connectionPoints1[z - 1].y,

                            x2 = connectionPoints2[z - 1].x,
                            y2 = connectionPoints2[z - 1].y,
